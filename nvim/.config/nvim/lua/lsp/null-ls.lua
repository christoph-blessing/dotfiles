...
local sources = {
    ...
    null_ls.builtins.formatting.rustfmt, null_ls.builtins.formatting.lua_format
}
    ...
    generator = helpers.generator_factory(
        {
            ...
            on_output = function(line, params)
                ...
                return find_entries(line, [[(%d+):(%d+):(%w):(.*)]],
                                    {"row", "col", "severity", "message"},
                                    severities)
            end
        })
...
local on_attach = function(client, bufnr)
    ...
    elseif client.resolved_capabilities.document_range_formatting then
        vim.api.nvim_buf_set_keymap(burnfr, "n", "<leader>f", "<cmd>lua vim.lsp.buf.range_formatting()<CR>", opts)
    end
end
